...
if (!exists("mapfile_bg")) mapfile_bg <- "data/gis/sample/sampleframe_maplist_31370.Rds"
if (!exists("samplingscript")) samplingscript <- "source/sample_design/grts_sample.R"


maplist_31370 <- readRDS(mapfile)
maplist_31370_bg <- readRDS(mapfile_bg)
...
plot_names <- ggplot2::geom_sf_text(data = maplist_31370_bg$map_antw,
                                    ggplot2::aes(label = NAAM),
                                    size = 4,
...
geoms_dist <- list(
  ggplot2::geom_sf(data = maplist_31370_bg$map_antw,
                   fill = INBOtheme::vl_yellow,
                   alpha = .3,
                   color = NA),
...
# geoms buffer area
geoms_buffer <- list(
  ggplot2::geom_sf(data = maplist_31370_bg$map_buff,
                   fill = INBOtheme::vl_darkyellow,
                   alpha = .3,
                   color = NA),
  ggplot2::geom_sf(data = maplist_31370_bg$map_buff_hab,
...
plot_dist <- plot_base + geoms_dist + ggplot2::theme_void() + plot_names
...
make_plot_dist_buff <- function(geoms_dist, geoms_buffer){
  plot_base + geoms_dist + geoms_buffer + plot_names +
    # emphasize dist
    ggplot2::geom_sf(data = maplist_31370_bg$map_antw_merged,
                     fill = NA,
                     color = INBOtheme::vl_black,
                     linewidth = 0.8) +
    ggplot2::theme_void()
